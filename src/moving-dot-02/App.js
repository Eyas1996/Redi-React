...
function App() {
  // create a set of state handlers
  // color and setColor with an initial state of "red"
  const [color, setColor] = useState("red");

  // Implement useState to toggle the display of the dot
  // (isOn, setOn with a default of true)
  const [inOn, setOn] = useState(true);

  // Implement useState to pass an object with x and y position from Canvas
  ...
  return (
    <>
      <Canvas color="#ddd" setPosition={setPosition}>
        <Dot isOn2={inOn} x={position.x} y={position.y} color2={color} />
      </Canvas>
      <h1>Moving Dot</h1>
